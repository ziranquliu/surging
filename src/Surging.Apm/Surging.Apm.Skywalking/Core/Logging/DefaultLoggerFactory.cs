...
using ILogger = Surging.Apm.Skywalking.Abstractions.Logging.ILogger;
using MSLoggerFactory = Microsoft.Extensions.Logging.LoggerFactory;

namespace Surging.Apm.Skywalking.Core.Logging
{
    public class DefaultLoggerFactory : Abstractions.Logging.ILoggerFactory
    {
        ...
        public DefaultLoggerFactory(IConfigAccessor configAccessor)
        {
            ...
            _loggerFactory.AddSerilog(new LoggerConfiguration().MinimumLevel.Verbose().Enrich
                ...
                .WithProperty(nameof(instrumentationConfig.ServiceName),
                    instrumentationConfig.ServiceName ?? instrumentationConfig.ServiceName).Enrich
